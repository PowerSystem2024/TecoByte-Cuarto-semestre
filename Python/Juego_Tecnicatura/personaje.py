...
class Explosion:
  def __init__(self,x,y):
    ...
    self.index = 0  # Indice de la animaciónp
    self.image = self.images[self.index]  # Imágen actual
    self.rect = self.image.get_rect(center=(x,y))  # Rectángulo de la imágen
    self.frame_rate = 0  # Contador de los frame de la animación
    self.max_frames = 20  # Frames por imagenes
  
  def actualizar(self):
    # Actualiza la animación del disparo
    self.frame_rate += 1
